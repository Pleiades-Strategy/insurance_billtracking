...
Airtable field names used
  ...
  Legislators:     Name, BT50 Party, Legislator Title, District,
                   State (from 2026 Insurance Bills),  In Office?,
                   Ballotpedia URL, FTM URL,  BT50 ID,
...
def to_list(val) -> list:
    """Normalise a scalar, None, or list to a plain list."""
    if val is None:
        return []
...
def linked_ids(fields: dict) -> list[str]:
    ...
    The linked-record API always returns an array of record-ID strings
    (e.g. ["recXXX", "recYYY"]).  We merge 2025 + 2026 fields so
    hearings and legislators work regardless of year.
    """
    return (
...
def transform_bills(records: list[dict]) -> list[dict]:
    ...
    for rec in records:
        ...
        # Category is a multi-select field → list of strings
...
def transform_legislators(records: list[dict]) -> list[dict]:
    """
    Field mapping (exact Airtable column names):
      Name                              → legislator_name  (includes party, e.g. "Ben Allen (D)")
      BT50 Party                        → legislator_party  ("D" / "R")
      Legislator Title                  → role              ("State Senator", etc.)
      District                          → district          ("CA024", etc.)
      State (from 2026 Insurance Bills) → state_code        (lookup → array; we take first)
      In Office?                        → in_office         (checkbox)
      Ballotpedia URL                   → ballotpedia_url
      FTM URL                           → follow_the_money_url
      BT50 ID                           → bt50_id
      2026 Insurance Bills              → linked_bill_ids   (linked record IDs for bill matching)
    """
    legislators = []
    for rec in records:
        f = rec.get("fields", {})

        # State is a lookup field — Airtable returns it as an array
        state_raw = to_list(fget(
            f,
            "State (from 2026 Insurance Bills)",
            "State (from 2025 Insurance Bills)",
            "State",
        ))
        state = str(state_raw[0]) if state_raw else ""
        ...
        legislators.append({
            ...
            "linked_bill_ids":      bill_ids,   # Airtable record IDs matching bills.bill_id
        })
    return legislators
...
def transform_hearings(records: list[dict]) -> list[dict]:
    ...
    Field mapping (exact Airtable column names):
      ...
      BillID               → bill_id_numeric  (BillTrack50 numeric ID — not used for linking)
